...
# Retrieving class from scientific name
  ...
  classes <- vector(length = nrow(idigbio))  
  ...
  sp.val <- vector() #do it with a set length
...
# body working, but time consuming
  ...
  # 
  # purrr::transpose(head(classif))
  # 
  # library(purrr)
  # 
  # classif[1:10] %>% flatten %>% map(~ split(.$name, .$rank)) %>% transpose %>% map(flatten_chr)
  ...
  class_name <- function(x) {
    for (i in 1:length(x)){
      if (is.na(x[[i]][[1]])){
        ...
        if (length(x[[i]][[1]][x[[i]][[1]][,2]=="class",1])==0){
        classes[i] <- "NA"
        } else {
          classes[i] <- x[[i]][[1]][x[[i]][[1]][,2]=="class",1]
        }
      }
    }
  }
  
  classes_teste <- class_name(classif) 
