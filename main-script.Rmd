...
data6$abundance<-1
data6$abundance<-as.integer(data6$abundance)
tt<-sqldf("select cells, count(Species) as N_rec from data6 group by cells having count(Species) > 0")
df7<-merge(x=data6, y=tt, by="cells", all.y=TRUE)
df7<-df7[df7$cells, c("ID", "Species", "abundance")] 
mat1<-data2mat(df7) 
total.slope<-specslope(x<-specaccum(mat1,method="exact"),(ceiling(length(x)-length(x)*0.9)))
plot(x,add=FALSE,random=FALSE,ci=2,ci.type="polygon",col=par("fg"),ci.col="grey",xlim=c(1,170),
...
sr<-sqldf("select cells, count (distinct(Species)) as sr_total from data6 group by cells")
```
