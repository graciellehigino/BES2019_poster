...
# Retrieving class from scientific name
  ...
  for (i in 1:nrow(idigbio)){
    sp[i] <- c(strsplit(as.character(idigbio$species[[i]]), ",")[[1]][1])
  }
  ...
  idb_spnames<-parLapply(c, sp, gnr_resolve ,data_source_ids=c(173,3,4,11,174,175),
                         resolve_once=FALSE,
...
#NOTE 20191112 - saved idigbio and idb_spnames in 'data/raw/idigbio.Rdata'. Load it next time.
##= NOT WORKING
  sp.val <- vector()
  unresolved <- vector()
  for (i in 1:length(idb_spnames)){
    if (length(idb_spnames[i][[1]]>0)){
      ...
      #if (length(attr(idb_spnames[i][[1]], "not_known"))>0) {
      unresolved[i] <-  attr(idb_spnames[i][[1]], "not_known")
      # } else {
      #   unresolved[i] <- NA
      # }
    }
...
##= NOT WORKING  
  ...
  t <- parLapply(c, idb_spnames,
          classification, db="gbif", rows=1)
  
  for (i in 1:end(sp)){
    classes[i] <- filter(t[[i]][[1]], rank=="class")[,1]
  }
