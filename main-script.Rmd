...
#TODO  # Split iDigBio into classes
  ...
  classes <- vector()  
  sp <- vector()
  for (i in 1:100){#nrow(idigbio)){
    sp[i] <- c(strsplit(as.character(idigbio$species[[i]]), ",")[[1]][1])
  }

  for (i in 1:100){#nrow(idigbio))
    classes[i] <- c(classification(sp[i], db = "gbif", rows = 1)[[1]][[1]][3])
  }
  
  idigbio$class <- classes # replace class column with new variable
  
  classes_list <- split.data.frame(idigbio, idigbio$class) #split df into classes
