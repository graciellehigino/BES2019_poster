...
download.file("http://biogeo.ucdavis.edu/data/worldclim/v2.0/tif/base/wc2.0_30s_bio.zip", mode="wb", destfile = "data/raw/temp.zip")
unzip("data/raw/temp.zip", exdir="data/raw")
temper <- raster("data/raw/whatever.bil")[[1]]
temper <- crop(temper, extente(ma))
...
## Get cells from raster
cells<-cellFromXY(temper, rasterize(ma, temper))

## Get variables from raster 
temp.p<-extract(temp,@coords,"simple")
alt.p<-extract(alt,data5@coords,"simple")
ndvi.p<-extract(ndvi,data5@coords,"simple")
pet.p<-extract(pet,data5@coords,"simple")
## Build df of variables
